...
import png
from function.subject import canvas2
import os
...
class ColorMultiImage(object):
    ...
    def generate(self, image_data, name, number, color_distance_filepath, coloring_style, colors_number):
        ...
        # print(range(0, len(np.asarray(image_data['data']).flatten())))
        # print([palette[np.asarray(image_data['data']).flatten()[x]] for x in
        #                     range(0, len(np.asarray(image_data['data']).flatten()))])
        image = np.asarray([palette[np.asarray(image_data['data']).flatten()[x]] for x in
                            range(0, len(np.asarray(image_data['data']).flatten()))]).flatten().reshape((24, 24, 3))
        # dirs = "../../static/upload/output/" + str(name) + "-output/"
        dirs = "../static/upload/" + str(name) + '/'
        print('-----------------------------------')
        print(dirs)
        print('-----------------------------------')
        ...
        cv2.imwrite(dirs + number + ".png", image)
